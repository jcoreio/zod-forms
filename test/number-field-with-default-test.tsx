...
it(`number field with default test`, async function () {
  const schema = z.strictObject({
    field: z.number().min(-10).max(10).default(5),
  })
  const onSubmit = sinon.spy<SubmitHandler<typeof schema>>(() => {})
  const { FormProvider, root, useFormContext, useInitialize, useSubmit } =
    createZodForm({
      schema,
    })
  ...
  await act(() => formContext?.setParsedValue(root.get('field'), 4))
  ...
  expect(onSubmit.args).to.deep.equal([
    [{ field: -9.5 }, { initialParsedValues: { field: 5 }, initialValues: {} }],
  ])
